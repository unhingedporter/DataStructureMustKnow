...
class Queue {
    ...
    dequeu() {
        return this.actualQueue.length > 0 ? this.actualQueue.splice(this.actualQueue.length - 1, 1)[0] : null
    }
}
    ...
    verticalOrderTraversal(root) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (!!treeNode.node.left) {
                queue.enqueu(new TreeNodeHd(treeNode.node.left, treeNode.horizontalDistance - 1))
            }

            if (!!treeNode.node.right) {
                queue.enqueu(new TreeNodeHd(treeNode.node.right, treeNode.horizontalDistance + 1))
            }
